...
| **I — failure-spectrum ansatz** | LER(p) fit (f5) over the sampled range | onset **fitted**; low-p extrapolation omitted |
| **III — replica-exchange splitting** | low-p LER + onset asymptote | absolute height needs the bridge fix (see below) |
""")

md(r"""## Setup — locate the repo and load the run
The first cell only imports. The second runs `bb6_report.compute(bb144_curve, models=("f5",))`, which
loads the npz + JSONs and fits the f5 ansatz with bootstrap bands (~15-30 s). It needs the
**completed** run.""")

code('''%load_ext autoreload
%autoreload 2
import sys, numpy as np, matplotlib.pyplot as plt
print("kernel python:", sys.executable)   # expect the 'qec' env (with `pip install -e .` run there)
import bb6_report                          # installed editable package — no sys.path hacks
from repo_paths import RUNS
# Full-spectrum IS sweep (w->115) + the replica-exchange splitting.json both live in bb144_curve_hi
# (the original bb144_curve was the under-sampled / wrong-splitter first pass).
...
R = bb6_report.compute(CURVE, models=("f5",))   # f5 only (f3 dropped); f0 unpinned -> ansatz fits onset
m = R["meta"]
...
for mod in R["LER"]:
    print(f"  {mod}: LER(1e-4) = {R['LER'][mod][np.argmin(abs(R['p_grid']-1e-4))]:.3e}  "
          f"(extrapolation past the sampled range is omitted on the plot — see Technique III for low p)")''')
...
# Literal pipes (the cardinality bars in |L(D)|) are column separators in a markdown table, so they
# MUST be escaped or the row gets shredded into the wrong cells.  chr(92) is a backslash.
esc = lambda s: str(s).replace("|", chr(92) + "|")
g = lambda k: ("—" if m.get(k) is None else f"{m[k]:.4g}" if isinstance(m[k],(int,float)) else str(m[k]))
fit = "fitted (odd D, no exact MITM)" if not m["f0_pinned"] else "exact"
rows = [
    ("circuit fault distance  D",   f"≤ {m['D']}" if m.get('distance_is_bound') else g('D'), "upper bound — BP-OSD search"),
    ("onset weight  w₀ = ⌈D/2⌉",    g('mw_onset_weight'),                                     "min-weight onset"),
    ("onset fraction  f₀",          f"{R['f0']:.3e}",                                         fit),
    ("|L(D)|  (compressed)",        f"≥ {g('n_min_logicals')}",                               "lower bound — search saturation"),
    ("|L(D)|  (expanded)",          f"≥ {g('n_min_logicals_expanded')}",                      "lower bound — search saturation"),
    ("Ñ  (compressed mechanisms)",  g('n_compressed'),                                        "single-sector DEM size"),
    ("N  (expanded mechanisms)",    g('n_expanded'),                                          "single-sector DEM size"),
]
header = "| quantity | value | note |\\n|---|---|---|\\n"
display(Markdown(header + "\\n".join(f"| {esc(a)} | {esc(b)} | {esc(c)} |" for a,b,c in rows)))
...
The main figure: per-weight importance-sampling failure fractions reweighted to LER(p), plus the f5
ansatz fit (f3 is dropped — f5 is the better fit) and the Technique-III splitting points.
...
> points), but its low-p tail collapses to an *impossibly* small LER (≈1e-58 at 1e-4) because it cannot
> see the w=6 onset — a severe *under*estimate. Pinning w0=6 instead would *over*estimate (the fit
> breaks).
>
> **So the plot draws the f5 ansatz only across the sampled range and omits its low-p extrapolation.**
> In its place, the green dotted line is the **onset asymptote** ∝ q^{w₀} (slope w₀=⌈D/2⌉=6, the true
> min-weight onset) with its absolute height pinned to the deepest Technique-III splitting rung. **For
> the actual low-p LER, read the splitting points / onset asymptote, not the ansatz.**""")
...
code('''if R["split"] is None:
    ...
    am = R.get("primary", "f5")
    print(f"{'p':>10} {'split P':>12} {'± bar':>11} {('ansatz '+am):>12} {'ratio':>7} {'consistent':>11}")
...
Median fault weight of the failing configs vs p (reweighted f5 ansatz), with the splitting chains'
...
out = REPO_ROOT / "notebooks" / "bravyi" / "bb12" / "BB144_failfast_report.ipynb"
