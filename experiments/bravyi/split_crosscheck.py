...
_CURVE = RUNS / "bravyi" / "bb6" / "bb6_fig10_curve"   # reads bb6_fig10.npz, writes splitting.json
...
def main():
    ...
    npz = np.load(_CURVE / "bb6_fig10.npz")
    ...
    out = {"tempered": {"p_ladder": pl.tolist(), "P_logical": tP.tolist(), "P_logical_se": tSE.tolist()},
           ...
           "compare": rows}
    (_CURVE / "splitting.json").write_text(json.dumps(out, indent=2))
    print(f"\nwrote {_CURVE / 'splitting.json'}")
