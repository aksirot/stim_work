...
fits the requested failure-spectrum ansatze (f3 and/or f5; see compute()'s `models`/`primary`),
...
DEFAULT_OUT = RUNS / "bravyi" / "bb6" / "bb6_fig10_curve"
K = 12
...
def compute(outdir=DEFAULT_OUT, bootstrap=150, seed=0, models=("f3", "f5"), primary=None):
    """Load results, fit the requested failure-spectrum ansatze (pinned w0/f0), compute LER(p) + bands.

    `models` selects which ansatze to fit (subset of {"f3","f5"}); pass `("f5",)` to skip f3
    entirely (f5 is the better fit and the only one bb144 reports). `primary` is the model used for
    the canonical w0/f0 meta, the splitting comparison, and the weight-distribution band; it
    ...
    if f0 is None:
        w0 = float(fits[primary].params.get("w0", w0))
        f0 = float(fits[primary].f(w0))
    ...
    return dict(w=w, F=F, T=T, N=N, q_base=q_base, f0=f0, w0=w0, p_ref=p_ref,
                ...
                primary=primary, split=split, repro=repro, meta=meta,
                search_conv=search_conv, dconv=dconv)
...
def onset_asymptote(p, R, anchor=None):
    """Low-p onset asymptote LER(p) ~ A·C(N,w0)·q^w0·(1-q)^(N-w0) using the TRUE Technique-II
    min-weight onset w0 = meta['mw_onset_weight'] (NOT the fitted/observable onset the ansatz
    settles on for odd D). The slope is therefore exactly w0=⌈D/2⌉; only the prefactor A (which
    folds in the onset failure fraction f0) is uncertain when D is odd.

    `anchor=(p*, P*)` pins A so the curve passes through that point — pass the deepest reliable
    splitting rung to read the absolute height off Technique III. With no anchor, A is taken as the
    pinned/exact f0 (valid only when the onset is exact, i.e. f0_pinned)."""
    w0 = int(R["meta"]["mw_onset_weight"]); N = R["N"]
    q = np.clip(R["q_base"] * (np.asarray(p, float) / R["p_ref"]), 1e-300, 1 - 1e-15)
    logC = gammaln(N + 1) - gammaln(w0 + 1) - gammaln(N - w0 + 1)
    log_shape = logC + w0 * np.log(q) + (N - w0) * np.log1p(-q)
    if anchor is not None:
        pa, Pa = anchor
        qa = float(np.clip(R["q_base"] * (pa / R["p_ref"]), 1e-300, 1 - 1e-15))
        log_shape_a = logC + w0 * np.log(qa) + (N - w0) * np.log1p(-qa)
        logA = np.log(max(Pa, 1e-300)) - log_shape_a
    else:
        logA = np.log(max(R["f0"], 1e-300))
    return np.exp(logA + log_shape)
...
def splitting_comparison(R):
    ...
    if R.get("repro"):
        ...
        bar = "se"
    ans = np.asarray(logical_error_rate_from_ansatz(R["fits"][R.get("primary", "f3")], list(tp)))
...
def fig_ler_vs_p(R, ax=None):
    ...
    isp = np.asarray(R["isp"])
    pinned = bool(R.get("meta", {}).get("f0_pinned", True))   # exact onset (even D) -> extrapolation valid
    in_rng = (p >= isp.min()) & (p <= isp.max())
    colors = {"f3": "crimson", "f5": "purple"}
    for model in R["LER"]:
        col = colors.get(model, "crimson")
        lo, hi = R["band"][model]
        # Exact onset (even D): the ansatz extrapolation IS the headline result — draw the full grid.
        # Fitted onset (odd D, e.g. bb144): the low-p tail is structurally unreliable (the fit can't
        # see the true min-weight onset ~40 weights below the sampled range, so it collapses to an
        # impossibly small LER and its bootstrap band pinches to an invisible thread). Draw the solid
        # line + band only across the SAMPLED range; the low-p height is handed to the onset asymptote
        # + splitting below.
        sel = slice(None) if pinned else in_rng
        lab = (f"Technique I: {model} ansatz (LER(1e-4)={R['LER'][model][np.argmin(abs(p-1e-4))]:.2e})"
               if pinned else f"Technique I: {model} ansatz (fit range only; low-p extrapolation omitted)")
        ax.fill_between(p[sel], lo[sel], hi[sel], color=col, alpha=0.15)
        ax.plot(p[sel], R["LER"][model][sel], "-", color=col, lw=2, label=lab)
    ...
    if R["split"]:
        ...
        # Onset asymptote: the physically-correct low-p slope q^w0 (w0=⌈D/2⌉) with its absolute
        # height pinned to the deepest splitting rung. Replaces the omitted ansatz tail when the
        # onset is fitted (odd D); for the exact-onset case the ansatz line already is this slope.
        if not pinned and tp.size:
            order = np.argsort(tp); pa, Pa = float(tp[order][0]), float(tP[order][0])
            pe = p[p <= isp.min()]
            if pe.size:
                w0t = int(R["meta"]["mw_onset_weight"])
                ax.plot(pe, onset_asymptote(pe, R, anchor=(pa, Pa)), ":", color="seagreen", lw=1.7,
                        label=fr"onset asymptote $\propto q^{{w_0}}$ ($w_0{{=}}{w0t}$, height pinned to splitting tail)")
...
def _weight_percentiles(R, p, qs=(0.1, 0.25, 0.5, 0.75, 0.9)):
    ...
    at physical rate p. The reweighting (binomial factor) is analytic; the f(w) input is the PRIMARY
...
def fig_weight_vs_p(R, ax=None):
    """Fig-9(c)-style: failing-config weight vs physical error rate. pi_q(w) band = the PRIMARY
    ANSATZ f(w) reweighted analytically by the binomial weight factor (semi-analytic: analytic
    reweighting, ansatz-fit input) + the replica-exchange chains' mean weight per ladder rate."""
    import matplotlib.pyplot as plt
    if ax is None: _, ax = plt.subplots(figsize=(8.4, 5.6))
    ...
    w0t = int(R.get("meta", {}).get("mw_onset_weight", R["w0"]))
    ax.axhline(w0t, color="darkorange", ls="--", lw=1.1,
               label=fr"Technique-II min-weight onset $w_0={w0t}$")
...
def fig_compare_ler(R_a, R_b, ax=None):
    ...
    for R, col in ((R_a, "crimson"), (R_b, "navy")):
        lab = _run_label(R)
        pm = R.get("primary", "f3")
        ax.plot(R["p_grid"], R["LER"][pm], "-", color=col, lw=2, label=f"{lab}: {pm} ansatz")
...
def fig_compare_spectrum(R_a, R_b, ax=None):
    ...
    for R, col in ((R_a, "crimson"), (R_b, "navy")):
        ...
        ax.plot(ww, R["fits"][R.get("primary", "f3")].f(ww), "-", color=col, lw=2, alpha=0.9)
...
def main():
    ...
    R = compute()
    for m in R["fits"]:
        print(f"{m} params:", {k: round(v, 4) for k, v in R["fits"][m].params.items()})
    for m in R["LER"]:
